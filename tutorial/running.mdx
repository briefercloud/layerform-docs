...
For that, you'll need to first learn how to set your CLI context so it know where to pull definitions from. Then, you'll have to run `layerform spawn` to create your environment.

Before we start, make sure you have installed the CLI using `go install`, as shown below.

```
$ go install github.com/ergomake/layerform
```

## Configuring your CLI context

Your CLI context is the place it pulls configurations from — the desired back-end.

In the last part of our tutorial, we've used an S3 bucket as the back-end for our layer definitions and state. Therefore, developers in your team should configure their Layerform CLI to pull configurations from that same bucket.

For that, they must update their `configuration.yml` file within the `.layerform` folder in their `home` folder.

```
$ vim ~/.layerform/configuration.yml
```

Within that file, you should create a context with the `type` `s3` and give it a name. Then, you should use that name for as the `currentContext`.
...
After that, your CLI will pull state and layer definitions from the `layerform-bucket-example`, which is the back-end you provisioned in the previous part of the tutorial.
